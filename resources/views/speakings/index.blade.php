...
<div class="page-header" style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 2.5rem; animation: fadeIn 0.5s ease-out;">
    <div>
        <h1 style="font-size: 2.25rem; font-weight: 800; color: var(--text-main); margin: 0 0 0.5rem 0; letter-spacing: -0.025em;">Speaking</h1>
        <p style="color: var(--text-secondary); margin: 0; font-size: 1rem;">Kelola soal speaking untuk mobile app</p>
    </div>
    <div style="display: flex; gap: 1rem; align-items: center;">
        <div class="search-box">
            <i data-lucide="search"></i>
            <input type="text" id="speaking-search" onkeyup="filterBySearch()" placeholder="Cari speaking...">
        </div>
...
<div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 1.25rem; margin-bottom: 2.5rem;">
    <div class="stat-card" style="background: white; padding: 1.5rem; border-radius: 1.25rem; border: 1px solid var(--border-color); box-shadow: var(--card-shadow);">
        <div style="color: var(--text-secondary); font-size: 0.8125rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 0.5rem;">Total Speaking</div>
        <div style="font-size: 1.75rem; font-weight: 700; color: var(--primary);">{{ $speakings->total() }}</div>
    </div>
    <div class="stat-card" style="background: white; padding: 1.5rem; border-radius: 1.25rem; border: 1px solid var(--border-color); box-shadow: var(--card-shadow);">
        ...
        <div style="font-size: 1.75rem; font-weight: 700; color: #6366f1;">{{ $jenisSpeakings->count() }}</div>
    </div>
</div>
    ...
    <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 1.5rem;">
        <div style="display: flex; align-items: center; gap: 1rem; flex: 1;">
            ...
            <div style="display: flex; gap: 0.625rem; flex-wrap: wrap;" id="theme-filters">
                <button class="filter-pill active" onclick="filterByTheme('all', this)">Semua ({{ $speakings->total() }})</button>
                @foreach($jenisSpeakings as $theme)
                    <button class="filter-pill" onclick="filterByTheme('{{ $theme->name }}', this)">{{ $theme->name }} ({{ $theme->speakings_count }})</button>
                @endforeach
            </div>
        </div>
    </div>
...
<div class="card" style="border-radius: 1.25rem; overflow: hidden; border: 1px solid var(--border-color); box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05);">
    <div class="table-container">
        <table id="speaking-table" style="border-collapse: separate; border-spacing: 0;">
            <thead>
                <tr>
                    <th style="padding: 1.25rem 1.5rem; border-bottom: 2px solid #f1f5f9; background: #f8fafc;">JENIS SPEAKING</th>
                    <th style="padding: 1.25rem 1.5rem; border-bottom: 2px solid #f1f5f9; background: #f8fafc;">JUDUL SOAL</th>
                ...
                @forelse($speakings as $speaking)
                    <tr class="speaking-row" data-theme="{{ $speaking->jenisSpeaking->name ?? 'none' }}">
                        ...
                        <td style="padding: 1.25rem 1.5rem; max-width: 400px; color: var(--text-secondary); line-height: 1.5; overflow-wrap: break-word; white-space: normal;">
                            {{ Str::limit($speaking->instruction, 100) }}
                        </td>
                        ...
                        <td style="padding: 1.25rem 1.5rem; color: var(--text-secondary); white-space: nowrap;">
                            {{ $speaking->created_at->format('Y-m-d') }}
                        </td>
                        <td style="padding: 1.25rem 1.5rem;">
                            <div style="display: flex; justify-content: center; gap: 0.75rem;">
                                <button class="action-btn btn-edit" onclick="openEditModal({{ $speaking->id }})" title="Ubah Data">
                                    <i data-lucide="edit-3"></i>
                                </button>
                                <button class="action-btn btn-delete" onclick="openDeleteModal({{ $speaking->id }})" title="Hapus Data">
                ...
                @empty
                    <tr id="empty-state">
                        <td colspan="6" style="text-align: center; padding: 5rem 2rem;">
                            ...
                            <p style="color: #94a3b8; font-size: 0.875rem; margin: 0;">Klik tombol di atas untuk menambah speaking pertama Anda.</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
...
@include('speakings.modals._create')
@include('speakings.modals._edit')
@include('speakings.modals._delete')
...
<script>
    ...
    function applyFilters() {
        const searchQuery = document.getElementById('speaking-search').value.toLowerCase();
        const rows = document.querySelectorAll('.speaking-row');
        let visibleCount = 0;
    ...
    function openEditModal(id) {
        fetch(`/speakings/${id}/edit`)
            .then(response => response.json())
            .then(data => {
                const form = document.getElementById('editForm');
                form.action = `/speakings/${id}`;
                
                // Set values
                document.getElementById('edit-title').value = data.speaking.title;
                document.getElementById('edit-instruction').value = data.speaking.instruction;
                document.getElementById('edit-point').value = data.speaking.point;
                
                // Select theme
                const themeSelect = document.getElementById('edit-theme-id');
                themeSelect.value = data.speaking.jenis_speaking_id;
